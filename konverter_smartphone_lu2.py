...
# Konfiguration
DOCX_FILE = "LOGIK_DES _UNIVERSUMS_WEB.docx"
OUTPUT_DIR = "LOGIK_DES_UNIVERSUMS_Smart2"
...
def create_mobile_book():
    ...
    for el in soup.contents:
        ...
        # Nur noch eine einzige Bedingung: Ist das Symbol ### im Element?
        if "###" in el_str:
            # Wir entfernen das ### aus dem Text, damit es im Buch nicht sichtbar ist
            clean_el_str = el_str.replace("###", "")
            
            # Den Text vor dem Symbol (falls vorhanden) noch zur aktuellen Seite hinzufügen
            current_content += clean_el_str
            
            # Seite abschließen und in die Liste einfügen
            if current_content.strip():
                pages.append(current_content)
                current_content = ""
            continue 

        # Alles andere wird einfach gesammelt, ohne die Seite zu brechen
...
.page-content { 
    ...
    font-size: 13px; line-height: 1.3; color: var(--text-color); 
    text-align: justify; 
}

img { max-width: auto; height: 80vh; display: block; margin: 15px auto; border-radius: 5px; }
.book-image{ max-width: 50vh; max-height: 40vh;}
...
"""
    ...
    with open(os.path.join(OUTPUT_DIR, "smart.html"), "w", encoding="utf-8") as f: f.write(index_html)
    with open(os.path.join(OUTPUT_DIR, "style-s.css"), "w", encoding="utf-8") as f: f.write(style_css)
    with open(os.path.join(OUTPUT_DIR, "script-s.js"), "w", encoding="utf-8") as f: f.write(script_js)
